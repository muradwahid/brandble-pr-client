...
const AdminPagination = ({ totalPages, currentPage, onPageChange }) => {
  ...
  return (
    ...
    >
      ...
      <button className="flex items-center text-sm px-4 rounded-md py-1 sm:text-sm font-normal border border-[#DCDEDF]">
        {currentPage}
      </button>
      ...
      <button
        ...
        className={`
        flex items-center px-1 py-1 pl-0 text-sm font-normal border border-[#DCDEDF] cursor-pointer rounded-md text-[#878C91]
        ${
          currentPage === 1
      ...
      >
        ...
        <span className="hidden sm:inline text-[#878C91]">Previous</span>
        <span className="sm:hidden text-[#878C91]">Prev</span>{" "}
        ...
        {getPageNumbers().map((page, index) =>
          typeof page === "number" ? (
            <button
              ...
              className={`
              font-normal rounded-md px-3 py-1 text-[#878C91] cursor-pointer
              ${
                currentPage === page
                  ? "bg-[#F2F2F3] text-[#36383A] " // Active page styling
                  : " text-[#878C91]" // Inactive page styling
              }
            ...
            <span
              key={index}
              className="px-2 py-1 text-[#878C91] cursor-pointer"
            >
              {page}
            </span>
          )
      ...
      <button
        ...
        className={`
        flex items-center px-1 py-1 pr-0 sm:text-sm font-normal cursor-pointer rounded-md text-[#878C91] text-sm border border-[#DCDEDF]
        ${
      ...
      `}
      >
        <span className="hidden sm:inline text-[#878C91]">Next</span>
        <span className="sm:hidden text-[#878C91]">Nxt</span>{" "}
        {/* Shorter text for small screens */}
        <svg
